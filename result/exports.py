...
def scores(request, pk, ty):
    ...
    if int(ty) == 1:
        ...
        if tutor.model_in == 'annual':
            ...
            return building(request, [data, [sum([int(float(df.Avg[i+1])) for i in range(len(df))]), round(mean([int(float(df.Avg[i+1])) for i in range(len(df))]), 2), len(df), tutor.Class, tutor.term+' MarkSheet', headers, tutor.term+' '+'Marksheet: '+tutor.subject.name+' '+tutor.subject.name+' '+tutor.Class, tutor.teacher_name]])
        else:
            return building(request, [data, [sum([int(float(df.Sum[i+1])) for i in range(len(df))]), round(mean([int(float(df.Sum[i+1])) for i in range(len(df))]), 2), len(df), tutor.Class, tutor.term+' MarkSheet', headers, tutor.term+' '+'Marksheet: '+tutor.subject.name+' '+tutor.Class, tutor.teacher_name]])
    else:
