...
from .models import BTUTOR, CNAME, QSUBJECT, Edit_User, QUESTION, STUDENT
#admin.site.register(studen_scores)#
########################################################
# Define the admin class
class subject_main(admin.ModelAdmin):
    list_display = ('student_id', 'student_name', 'tutor', 'term')
...
class model_teacher(admin.ModelAdmin):
    list_display = ('accounts', 'teacher_name', 'subject', 'Class', 'term', 'session')
...
class model_names(admin.ModelAdmin):
    list_display = ('uid', 'Class','term','full_name', 'created', 'updated', 'session')
...
class model_profile(admin.ModelAdmin):
    ...
    fields = [('title', 'user', 'account_id'), ('photo', 'image'), 'bio', ('phone', 'city', 'country', 'organization', 'location', 'birth_date', 'department'), 'email_confirmed', ('session', 'resumption')]
